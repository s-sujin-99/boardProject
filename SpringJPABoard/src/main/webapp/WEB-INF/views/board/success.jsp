        ...
        <c:if test="${not empty boardDto}">
            <div class="notion-preview-block">
                <div class="preview-item">
                    ...
                    <span class="preview-value">${boardDto.title}</span>
                </div>
                <div class="preview-item">
                    <span class="preview-label">👤 작성자</span>
                    <span class="preview-value-badge">${boardDto.writer}</span>
                </div>
            </div>
        </c:if>

        <div class="notion-action-links">
            <a href="/board/list" class="notion-link-btn primary">📁 전체 목록 보기</a>
            <c:if var="hasNo" test="${boardDto.boardNo > 0}">
                <a href="/board/select?boardNo=${boardDto.boardNo}" class="notion-link-btn">생성된 페이지 확인</a>
            </c:if>
        </div>
